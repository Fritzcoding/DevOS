...
function classifyByHeuristics(relPath) {
    const normalized = relPath.replace(/\\/g, '/').toLowerCase();
    const ext = path_1.default.posix.extname(normalized);
    ...
    if (normalized.includes('research') || normalized.includes('paper'))
        category = 'research';
    if (normalized.includes('invoice') || normalized.includes('receipt') || normalized.includes('tax'))
        category = 'financial';
    if (normalized.includes('academic') || normalized.includes('university') || normalized.includes('thesis'))
        category = 'academic';
    if (normalized.includes('screenshot'))
        tags.add('screenshot');
    if (normalized.includes('typescript') || ['.ts', '.tsx'].includes(ext))
        tags.add('typescript');
    if (normalized.includes('electron'))
        tags.add('electron');
    if (normalized.includes('react') || ext === '.tsx' || ext === '.jsx')
        tags.add('react');
    if (normalized.includes('docker'))
        tags.add('docker');
    if (normalized.includes('kubernetes') || normalized.includes('k8s'))
        tags.add('kubernetes');
    if (normalized.includes('tmp') || normalized.includes('temp') || normalized.endsWith('.bak'))
        category = 'temporary';
    return {
        category,
