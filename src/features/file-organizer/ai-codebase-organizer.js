...
exports.generateOrganizerPlan = generateOrganizerPlan;
exports.validateOrganizationPlan = validateOrganizationPlan;
...
function inferLayer(relPath) {
    ...
    if (p.includes('/controller') || p.includes('controller.'))
        return 'controller';
    if (p.includes('/repository') || p.includes('repository.'))
        return 'repository';
    if (p.includes('/service') || p.includes('service.'))
        return 'service';
    if (p.includes('/ui/') || p.includes('/component') || p.endsWith('.tsx'))
        return 'ui';
    if (p.includes('/infra') || p.includes('/infrastructure'))
        return 'infrastructure';
    if (p.includes('/db/') || p.includes('/database'))
        return 'database';
    return 'unknown';
}
...
function parseTSLike(file, content) {
    const source = typescript_1.default.createSourceFile(file.relPath, content, typescript_1.default.ScriptTarget.Latest, true);
    const dependencies = [];
    const classes = [];
    const methods = [];
    const interfaces = [];
    const walk = (node) => {
        ...
        if (typescript_1.default.isClassDeclaration(node) && node.name)
            classes.push(node.name.getText(source));
        if (typescript_1.default.isInterfaceDeclaration(node) && node.name)
            interfaces.push(node.name.getText(source));
        if ((typescript_1.default.isMethodDeclaration(node) || typescript_1.default.isFunctionDeclaration(node)) && node.name) {
...
function parseJava(file, content) {
    ...
    for (const raw of lines) {
        ...
        if (line.startsWith('import '))
            dependencies.push(line.replace('import ', '').replace(';', '').trim());
        if (line.startsWith('@'))
            annotations.push(line.slice(1).split(/[ (]/)[0]);
        const classMatch = line.match(/\b(class|interface)\s+([A-Za-z0-9_]+)/);
        if (classMatch)
            classes.push(classMatch[2]);
...
function parseFile(file, content) {
    ...
    if (file.ext === '.ts' || file.ext === '.tsx' || file.ext === '.js' || file.ext === '.jsx')
        return parseTSLike(file, content);
    return {
        id: file.relPath,
        path: file.relPath,
        language: 'text',
...
function resolveInternalEdges(nodes) {
    const pathByLeaf = new Map();
    for (const node of nodes) {
        const base = path_1.default.basename(node.path, path_1.default.extname(node.path));
...
function architectureViolations(nodes) {
    const violations = [];
...
function suggestMoves(files, nodes) {
    const moves = [];
    ...
    for (const file of files) {
        ...
        const base = path_1.default.posix.basename(rel);
        const ext = file.ext.toLowerCase();
        const topLevel = !rel.includes('/');
        if (/(_backup|_old|\.bak)$/i.test(base))
            continue;
        let target = null;
        if (topLevel && !protectedTop.has(base)) {
            if (['.md', '.txt', '.adoc'].includes(ext))
                target = `docs/${base}`;
            if (['.png', '.jpg', '.jpeg', '.svg', '.webp', '.ico'].includes(ext))
                target = `assets/${base}`;
            if (['.sh', '.ps1', '.bat', '.cmd'].includes(ext))
                target = `scripts/${base}`;
        }
...
function hasExplicitProfessionalIntent(instruction) {
...
function wantsSnakeCaseRename(instruction) {
    return /\b(rename|clean names?|poorly formatted|snake_case|snake case)\b/i.test(instruction);
}
function toSnakeCaseFilename(filename) {
    const ext = path_1.default.posix.extname(filename).toLowerCase();
...
function categoryDirFromInstruction(instruction, rule) {
...
function buildExplicitInstructionPlan(files, instruction) {
    const moves = [];
    ...
    const addMove = (fromRaw, toRaw, reason) => {
        const from = normalizeRel(fromRaw);
        const to = normalizeRel(toRaw);
        const source = byPath.get(from.toLowerCase());
        if (!source || !to || to.endsWith('/'))
            return;
        if (path_1.default.posix.basename(from) !== path_1.default.posix.basename(to))
            return;
...
function buildInstructionCategoryPlan(files, instruction) {
    if (!hasExplicitProfessionalIntent(instruction))
        return [];
    const lower = instruction.toLowerCase();
    const moves = [];
    const existing = new Set(files.map((file) => file.relPath.toLowerCase()));
    ...
    const addByExt = (exts, targetDir, reason) => {
        for (const file of files) {
            const base = path_1.default.posix.basename(file.relPath);
            const ext = file.ext.toLowerCase();
            const financeDocument = targetDir.toLowerCase() === 'financials' &&
                ['.pdf', '.doc', '.docx', '.txt'].includes(ext) &&
                FINANCIAL_FILENAME_PATTERN.test(base);
            if (!exts.includes(ext) && !financeDocument)
                continue;
            if (PROTECTED_ROOT_FILES.has(base))
                continue;
            if (targetDir.toLowerCase() === 'documents' &&
                ['.pdf', '.doc', '.docx', '.txt'].includes(ext) &&
            ...
            if (file.relPath === target || existing.has(targetKey) || plannedDestinations.has(targetKey))
                continue;
            if (file.relPath.includes('/') && !allowNested)
                continue;
...
function findRedundant(files) {
    const out = [];
    const bySize = new Map();
    for (const f of files) {
        if (f.size > 100)
            bySize.set(f.size, [...(bySize.get(f.size) || []), f.relPath]);
        if (/(_backup|_old| copy|\.bak)$/i.test(path_1.default.posix.basename(f.relPath))) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                if (!IGNORED_DIRS.has(entry.name))
                    await walk(path_1.default.join(dir, entry.name));
                continue;
            }
            if (!entry.isFile())
                continue;
            ...
            files.push({ relPath, absPath: abs, size: stat.size, ext: path_1.default.extname(entry.name).toLowerCase() });
        }
    };
    await walk(root);
    return files;
}
async function generateOrganizerPlan(rootDir, instruction, mode = 'professional') {
    ...
    for (const file of semanticCandidates) {
        const content = await fs_extra_1.default.readFile(file.absPath, 'utf8').catch(() => '');
    ...
    const newDirs = Array.from(new Set(movePlan.map((m) => path_1.default.posix.dirname(m.to)).filter((d) => d && d !== '.')));
...
function validateOrganizationPlan(rootDir, organization) {
    const errors = [];
    const root = path_1.default.resolve(rootDir);
    const destinations = new Set();
    const checkRel = (value, label) => {
        const candidate = path_1.default.resolve(root, value || '');
    ...
    for (const move of organization?.moves || []) {
        ...
        if (path_1.default.posix.basename(from.replace(/\\/g, '/')) !== path_1.default.posix.basename(to.replace(/\\/g, '/')) && organization?.allow_renames !== true) {
