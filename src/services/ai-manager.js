...
class AIManager {
    constructor() {
        ...
        this.modelChain = ['gemini', 'openai', 'ollama'];
        this.modelStats = {
            gemini: { attempts: 0, successes: 0, lastError: null },
            openai: { attempts: 0, successes: 0, lastError: null },
            ollama: { attempts: 0, successes: 0, lastError: null },
        };
     ...
     */
    async fixCode(code, language, prompt, preferredModel) {
...
Please fix and improve this code.`;
        ...
        for (const model of chain) {
            ...
            try {
                ...
                console.log(`[AI Manager] ✅ ${model}: Success in ${duration}ms (${this.modelStats[model].successes}/${this.modelStats[model].attempts})`);
                return {
                    text: response,
                    model,
                    duration,
                ...
                console.warn(`[AI Manager] ⚠️  ${model}: Failed - ${errorMsg}. Trying next model...`);
        ...
        throw new Error(`All AI models failed. Gemini: ${errors.gemini}. OpenAI: ${errors.openai}. Ollama: ${errors.ollama}`);
    }
    /**
     * Chat with AI using fallback chain
     */
    async chat(message, context = '', preferredModel) {
...
Be concise but thorough in your explanations.`;
     ...
     * Call specific model
     */
    async callModel(model, systemPrompt, userMessage) {
        switch (model) {
            case 'gemini':
...
exports.default = exports.aiManager;
